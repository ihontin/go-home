...
func (c *UserCollection) UniqByNickname() *UserCollection {
	collection := &UserCollection{}

	for _, item := range c.Items {
		searchItem := collection.Find(func(model *User) bool {
			return model.Nickname == item.Nickname
		})
...
type byNicknameAsc []*User

func (a byNicknameAsc) Len() int           { return len(a) }
func (a byNicknameAsc) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a byNicknameAsc) Less(i, j int) bool { return a[i].Nickname < a[j].Nickname }

type byNicknameDesc []*User

func (a byNicknameDesc) Len() int           { return len(a) }
func (a byNicknameDesc) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a byNicknameDesc) Less(i, j int) bool { return a[i].Nickname > a[j].Nickname }

func (c *UserCollection) SortByNickname(mode string) *UserCollection {
	collection := &UserCollection{Items: c.Items}

	if mode == "desc" {
		sort.Sort(byNicknameDesc(collection.Items))
	} else {
		sort.Sort(byNicknameAsc(collection.Items))
	}
